...
class Slide:
    ...
    def slide_img(self, src='', host='', filepath="../lib/imgs/t.png"):
        """
        下载验证码图片
        :param src: 图片
    ...
    def slide_verify(self, xpath1, xpath2, jsonp=None):
        ...
        self.base_param = {
            'zoom': 1,
            'target': './lib/images/target.png',
            'template': './lib/images/template.png',
            'host': '',
            'offset-x': 0,
            'offset-y': 0,
            'slide': 'selenium',
        ...
        while i < 10:
            ...
            if i > 0 and succ == False:
                ...
                if self.base_param["refresh"] != "":
                    eler = self.__find_ele(self.base_param["refresh"])
                    eler.click()
                    time.sleep(2)
            ...
            if self.base_param['slide'] == 'selenium':
                # 计算缩放比例，计算偏移量
                x = int(x * w1 / w2) - self.base_param['offset-x']
    ...
    def slide_by_pyautogui(self, x, y, offset):
        ...
        pyautogui.moveTo(x + int(offset * random.randint(17, 21) / 20), y,
                         duration=(random.randint(20, 51)) / 100)  # 鼠标拖动到坐标(1566,706)位置处
